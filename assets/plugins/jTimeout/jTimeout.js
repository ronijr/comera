...
(function ($) {
                ...
                if (seconds < 0 && !timeout.timedOut)
                {
                    ...
                    if (!options.onTimeout)
                    {
                        ...
                        $.jAlert({
                            'id': 'jTimedoutAlert',
                            'title': 'Oops!',
                            'content': 'Sesi masuk Anda sudah berakhir.',
                            'theme': 'red',
                            // 'blurBackground': true,
                            'btns': {
                                'text': 'Masuk Kembali',
                ...
                else if (seconds < options.secondsPrior && !timeout.timeoutWarning)
                {
                    ...
                    if (!options.onPriorCallback) {
                        $.jAlert({
                            'id': 'jTimeoutAlert',
                            'title': 'Warning',
                            'content': '<b>Sesi akan berakhir dalam <span class="jTimeout_Countdown">' + seconds + '</span> detik!</b>',
                            'theme': 'red',
                            'blurBackground': true,
                            'closeBtn': false,
    ...
    $.jTimeout.defaults = {
        ...
        'extendUrl': BASE_URL+'dashboard', //URL to request in order to extend the session.
        'logoutUrl': BASE_URL+'auth/logout', //URL to request in order to force a logout after the timeout. This way you can end a session early based on a shorter timeout OR if the front-end timeout doesn't sync with the backend one perfectly, you don't look like an idiot.
        'loginUrl': BASE_URL+'auth/login', //URL to send a customer when they want to log back in
